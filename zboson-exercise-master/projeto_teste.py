...
if escolha == 1:
    ...
    initials_z =  [float(x) for x in input('Preencha com os parâmetros da Breit-Wigner (gamma, M, a, b, A) dando apenas espaços entre eles: ').split()]
    ...
    # Critério de avaliação dos ajustes: Iterações e chi2 normalizado.
    ...
    print("Baseado no valor de chi2 dividido pelo número de graus de liberdade: ")
...
################################################## UPSILON ################################################# 
    ...
    initials_upsilon =  [float(x) for x in input('Preencha com os parâmetros das duas Gaussianas e da exponencial (a1, mean1, sigma1, a2, mean2, sigma2, constante, inclinação): ').split()]
...
################################################### J/PSI ################################################## 
    ...
    initials_jpsi =  [float(x) for x in input('Preencha com os parâmetros da Crystal-Ball e da exponencial (a, n, mean, sigma, constante, inclinação): ').split()]
...
################################################### PSI' ################################################### 
    ...
    initials_psiprime =  [float(x) for x in input('Preencha com os parâmetros da Crystal-Ball e da exponencial (a, n, mean, sigma, constante, inclinação): ').split()]
